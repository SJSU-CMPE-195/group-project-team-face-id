# This script tests enrollment and shows live debug for norm and face count.
import pickle
...
def main():
    ...
    try:
        while len(collected_embeddings) < samples_needed:
            ...
            # Picamera2 returns RGB888; convert for OpenCV display and processing
            ...
            if len(faces) == 1:
                ...
                cv2.putText(display_frame, "1 face detected",  (10, 30),  cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
                cv2.putText(display_frame, f"Norm: {norm:.2f}  Shape: {shape}", (10, 110), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)

            elif len(faces) == 0:
                cv2.putText(display_frame, "No face detected",       (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
            else:
                cv2.putText(display_frame, "Multiple faces detected", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)

            cv2.putText(display_frame, status_text, (10, 70), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
